...
WITH amount_sum_months AS (
    SELECT
        DATE_TRUNC('month', book_date) AS month_date,
...
SELECT
    month_date::date,
    ROUND(total_monthly_amount / LAG(total_monthly_amount) OVER (
        ORDER BY month_date
    ) * 100, 2)
    - 100
    AS percent_diff
...
seats_cumulative AS (
    SELECT
        ...
        COUNT(s.seat_no) AS empty_seats_per_aircraft,
        SUM(COUNT(s.seat_no))
            OVER (
                PARTITION BY f.departure_airport, f.actual_departure::date
                ORDER BY f.actual_departure
            )
        AS seats_cum_sum,
        COUNT(f.actual_departure::date) OVER (
            PARTITION BY
                f.departure_airport,
                f.actual_departure::date
        ) AS days_count_by_airport
    FROM flights AS f
    INNER JOIN empty_flights AS ef ON f.flight_id = ef.flight_id
...
SELECT
    departure_airport,
    actual_departure,
    empty_seats_per_aircraft,
    seats_cum_sum
FROM seats_cumulative
...
SELECT
    ad.airport_name AS dept_airport_name,
    aa.airport_name AS arr_airport_name,
    COUNT(f.flight_id)
    * 100
    / SUM(COUNT(f.flight_id)) OVER () AS flights_percent_rate
FROM flights AS f
INNER JOIN airports AS ad ON f.departure_airport = ad.airport_code
...
WITH phone_codes AS (
    SELECT
        SUBSTRING(
            contact_data
            ->> 'phone' FROM POSITION('+7' IN contact_data ->> 'phone')
            + 2 FOR 3
        )
        AS cell_code
    FROM tickets
...
SELECT
    cell_code,
    COUNT(cell_code) AS passengers_num
FROM phone_codes
GROUP BY cell_code;
...
SELECT
    turnover_class,
    COUNT(turnover_class) AS routes_sum
FROM (
    SELECT
        CASE
            WHEN SUM(amount) < 50000000 THEN 'low'
            WHEN
                SUM(amount) >= 50000000 AND SUM(amount) < 150000000
                THEN 'middle'
            ELSE 'high'
        END AS turnover_class
    FROM ticket_flights AS ft
    INNER JOIN flights AS f ON ft.flight_id = f.flight_id
...
SELECT ROUND((mba.med_book_amnt / mfa.med_flght_amnt)::numeric, 2) AS median_rate
...
WITH distance_between_airports_km AS (
    SELECT
        CONCAT(a1.airport_code, a2.airport_code) AS joint_apt_id,
        EARTH_DISTANCE(
            LL_TO_EARTH(a1.latitude, a1.longitude),
            LL_TO_EARTH(a2.latitude, a2.longitude)
        ) / 1000 AS airport_distance_km
    FROM airports AS a1, airports AS a2
    WHERE a1.airport_code != a2.airport_code
),

min_amount_between_airports AS (
    SELECT
        CONCAT(f.departure_airport, f.arrival_airport) AS joint_apt_id,
        MIN(tf.amount) AS min_route_amount
    FROM flights AS f
...
SELECT
    ROUND(
        (min_route_amount / airport_distance_km)::numeric, 2
    ) AS min_flight_rate_per_km
FROM distance_between_airports_km AS dbak
INNER JOIN
    min_amount_between_airports AS maba
    ON dbak.joint_apt_id = maba.joint_apt_id
