...
class RsQueries:
    ...
    def get_hybrid_recommendations(
        self, target_sku, top_n=5, weight_cbf=0.6, weight_cf=0.4
    ):
        ...
        res.to_excel("hybrid_recommendations.xlsx", index=False)
        return res

    def evaluate_model_precision(self, model, df_sim, top_n=10):
        precisions = []
        ...
        for query, group in sales_queries.groupby("query_cluster_repr"):
            ...
            for sku in relevant_skus:
                ...
                recommended_skus = set(recs_df["sku"].tolist())
                relevant_recs = recommended_skus & relevant_skus
                precision = len(relevant_recs) / min(top_n, len(recommended_skus))
...
if __name__ == "__main__":
    ...
    # df_mod = rs_model.eda()
    rs_model.eda_plot()
    df_sim = rs_model.cluster_similar_queries()
    # rs_model.get_similar_skus(1614691389, top_n=10)
    # rs_model.get_similar_skus_collab(775129914, top_n=10)
    hybrid_model = rs_model.get_hybrid_recommendations(
        1614691389, top_n=5, weight_cbf=0.6, weight_cf=0.4
    )

    # cbf_precision = rs_model.evaluate_model_precision(
    #     lambda sku, top_n: rs_model.get_similar_skus(sku, top_n),
    #     df_sim,
    #     top_n=10
    # )
    #
    # cf_precision = rs_model.evaluate_model_precision(
    #     lambda sku, top_n: rs_model.get_similar_skus_collab(sku, top_n),
    #     df_sim,
    #     top_n=10
    # )
    #
    # hybrid_precision = rs_model.evaluate_model_precision(
    #     lambda sku, top_n: rs_model.get_hybrid_recommendations(sku, top_n, weight_cbf=0.6, weight_cf=0.4)[['sku']],
    #     df_sim,
    #     top_n=2
    # )

    # print(f"CBF Precision@10: {cbf_precision.round(2)}")
    # print(f"CF Precision@10: {cf_precision.round(2)}")
    # print(f"Hybrid Precision@10: {hybrid_precision.round(2)}")
